...
<head>
    ...
    <title>Hagos ERP - Penerimaan Uang</title>
...
<div class="min-h-screen p-6">
    <header class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Penerimaan Uang</h1>
        <p class="text-gray-500 mt-1">Pemasukan di luar order penjualan (jual tester, patungan, dll). Masuk kas & dihitung sebagai Pendapatan Lain-lain di P&L. Tidak memotong stok.</p>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-sm p-5 sticky top-6">
                <h2 class="font-bold text-emerald-700 mb-4">➕ Catat Penerimaan</h2>
                <form method="POST" action="{{ route('penerimaan.store') }}" class="space-y-3">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Kategori / Alasan <span class="text-red-500">*</span></label>
                        <input type="text" name="kategori" list="kat-list" required placeholder="cth: Jual Tester"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Masuk ke Akun <span class="text-red-500">*</span></label>
                        <select name="akun" required class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                    ...
                    <div>
                        ...
                        <select name="oleh" class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                    ...
                    <div>
                        ...
                        <textarea name="keterangan" rows="2" placeholder="opsional..." class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"></textarea>
                    </div>
                    <button type="submit" class="w-full bg-emerald-600 text-white py-2 rounded-md font-semibold hover:bg-emerald-700">Simpan Penerimaan</button>
                </form>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-5">
            <div class="bg-white rounded-xl shadow-sm p-5">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <span class="text-sm font-bold text-emerald-600">Total: Rp {{ number_format($totalSemua, 0, ',', '.') }}</span>
                </div>
                @if($perJenis->isEmpty())
            ...
            <div class="bg-white rounded-xl shadow-sm p-4">
                <form method="GET" action="{{ route('penerimaan.index') }}" class="flex flex-wrap items-end gap-3">
                    <div>
                    ...
                    @if($dari || $sampai || $jenis)<a href="{{ route('penerimaan.index') }}" class="px-4 py-1.5 rounded-md text-sm font-semibold bg-gray-100 text-gray-600 hover:bg-gray-200">Reset</a>@endif
                </form>
            </div>

            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-100 flex justify-between items-center">
                    <h2 class="font-bold text-gray-800 text-sm">Riwayat Penerimaan</h2>
                    <span class="text-sm text-gray-500">Subtotal tampil: <strong class="text-emerald-600">Rp {{ number_format($totalFilter, 0, ',', '.') }}</strong></span>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50"><tr>
                            ...
                            <th class="px-4 py-2 text-left text-xs text-gray-500 uppercase">Catatan</th>
                            <th class="px-4 py-2 text-left text-xs text-gray-500 uppercase">Akun</th>
                            <th class="px-4 py-2 text-left text-xs text-gray-500 uppercase">Oleh</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse($penerimaans as $p)
                                @php $parts = explode('·', $p->keterangan, 2); $jns = trim($parts[0]); $cat = isset($parts[1]) ? trim($parts[1]) : ''; @endphp
                                <tr>
                                    <td class="px-4 py-2 text-gray-600 whitespace-nowrap">{{ \Illuminate\Support\Carbon::parse($p->tanggal)->format('d/m/Y') }}</td>
                                    <td class="px-4 py-2"><span class="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded">{{ $jns }}</span></td>
                                    ...
                                    <td class="px-4 py-2 text-right font-semibold text-emerald-600 whitespace-nowrap">Rp {{ number_format($p->jumlah, 0, ',', '.') }}</td>
                                    <td class="px-4 py-2 text-center">
                                        <form method="POST" action="{{ route('penerimaan.destroy', $p->mutasi_id) }}" onsubmit="return confirm('Hapus penerimaan ini? Saldo akun disesuaikan.')">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400 italic">Belum ada penerimaan.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="px-4 py-3 border-t border-gray-100">{{ $penerimaans->links() }}</div>
            </div>
        </div>
    </div>
</div>
